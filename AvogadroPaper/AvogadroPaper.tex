...
is a framework providing code libraries and application programming interface (API)
...
project named ``OpenQube''.
The ``OpenQube'' library has since been added as an optional backend in
...
Avogadro uses the PDB reader from Open Babel to visualize the
secondary structure annotated in those files. Two plugins exist for
the rendering of this information.  The first is a simple plugin which
renders a simple tube between the biomolecule backbone.  A 
second more advanced visualization plugin calculate meshes for
the alpha helices and beta sheets. While the simple plugin is much
...
The choice of license is an interesting one and hotly debated in many
...
{\tt Bond}, views are made up of the engine/render plugins, and 
...
At start up, several standard directories--which may be
customized--are searched for plugins. The Qt plugin
framework is used to check that the plugins have a recent enough
version to be loaded, and the plugin type can be deduced once
loaded. The user interface is then populated with appropriate
entries; tools are added to the main toolbar using their embedded
icons, display types are added to the display type list, and menu
...
cylinders, typically used to represent atoms and bonds. But there are many other
properties that can be rendered using the display type plugins.
For example, some of the engines also convey information about the underlying data the
...
since multiple display types can be
...
The navigation tool provides basic scene navigation, implementing
rotation, panning, tilting, and zooming support. Depending on the
point of interaction (where the initial click occurs) changes the
anchor for navigation; navigation takes place about the center of
molecule when clicking in empty space or about the center of any
clicked atom. During interaction, the navigation tool provides visual
cues to show what type of navigation is taking place. The navigation
tool is also used as the default tool if the currently active tool
...
structures is required are required and these structures are treated
as read-only. This allows the calculation to be
multithreaded and share one copy of the input data. The scalar value of
...
calculate. A proper implementation would offload this calculation to
the GPU is likely to be more useful as the result would already be in
